...
class MyMotifs:
    ...
    def maskedConsensus(
            self):  # vai procurar o masked consensus que é a consensus mas só com as letras que tem uma incidência maior do que 50% em todas as seqs
        res = ""
        for j in range(self.size):
            maxcol = self.counts[0][j]
            maxcoli = 0
    ...
    def probAllPositions(self,
                         seq):  # este em vez de calcular a probabilidade de acontecer devolve uma lista com as probabilidades de acontecer em cada letra da seq
        res = []
        for k in range(len(seq) - self.size + 1):
            res.append(self.probabSeq(seq))
        return res

    def mostProbableSeq(self,
                        seq):  # vai ver qual a posição inicial da subseq de uma seq de comprimento indefenido encaixa melhor no quandro de motifs das seqs
        maximo = -1.0
        maxind = -1
