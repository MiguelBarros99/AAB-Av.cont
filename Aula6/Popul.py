...
class Popul:
    ...
    def selection(self, n, indivs=None):  # mecanismo de selecao para reproducao
        res = []
        fitnesses = list(
            self.linscaling(self.getFitnesses(indivs)))  # vai obter os fitnesses dos individuos e fazer a normalizacao
...
class PopulInt(Popul):
    ...
    def initRandomPop(self):
        self.indivs = []
        for _ in range(self.popsize):  # quantidade de listas a gerar
            indiv_i = IndivInt(self.indsize, [], 0,
                               self.ub)  # diferenca IndivInt #gerar os individuos aleatoriamente (n de elemntos, lista)
            self.indivs.append(indiv_i)  # adicionar os individuos a lista
